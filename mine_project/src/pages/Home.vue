      ...
      <main class="xl:px-[80px] md:px-[50px] px-[20px]">
        ...
        <!-- <div class='flex justify-center md:flex hidden'>
          ...
          <nav class="bg-red-400 text-white grid grid-flow-col text-center mt-14">
            <div @click="filterBook(all)" class="cursor-pointer hover:bg-white hover:text-red-400 transition-all duration-500 w-[120px] p-3 active">All recipes</div>
            <template  v-for="category in categories" :key="category.id">
              <div @click="filterBook(category.name)" class="cursor-pointer hover:bg-white hover:text-red-400 transition-all duration-500 w-[120px] p-3 active">{{ category.name }}</div>
